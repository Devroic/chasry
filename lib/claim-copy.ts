...
export const claimCopy = {
  ...
  intro: (businessName: string, locale: Locale) =>
    locale === "el"
      ? `Δηλώστε ότι έχετε ήδη στείλει την πληρωμή για αυτό το τιμολόγιο. Ο αποστολέας (${businessName}) θα ενημερωθεί και οι υπενθυμίσεις θα σταματήσουν μέχρι να επιβεβαιωθεί η παραλαβή της.`
  ...
  doneBody: (businessName: string, locale: Locale) =>
    locale === "el"
      ? `Ο αποστολέας (${businessName}) ενημερώθηκε. Δεν χρειάζεται να κάνετε κάτι άλλο.`
  ...
  alreadyPaidBody: (businessName: string, locale: Locale) =>
    locale === "el"
      ? `Ο αποστολέας (${businessName}) έχει ήδη σημειώσει αυτό το τιμολόγιο ως πληρωμένο. Ευχαριστούμε!`
